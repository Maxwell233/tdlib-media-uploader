...
V1.8.6 默认 missing_date_policy = "mtime"，因此没有 ExifTool 也能上传视频。
...
日期规则：
- 优先 EXIF/QuickTime；日期缺失或未安装 ExifTool 时，通过 FFmpeg 读取媒体创建日期。
- 内部日期都缺失时，mtime 策略使用文件修改日期，error 策略停止；不使用文件创建时间。
