...
- 混合上传以目录下的一级子文件夹为组，递归收集其中的图片和视频并按相对路径排序；每组可设置 1–10 个媒体，超过数量会拆成多个 Album，组标题默认使用文件夹名。
...
| 扫描排序 | 修改时间或文件名二选一 | 修改时间或完整路径二选一 | 相对路径排序 |
...
混合上传单独显示在侧栏。`mixed_dir` 下每个一级子文件夹是一个独立组，组内可同时放入图片和视频（也支持更深层目录）；同组媒体保持统一顺序并使用 Telegram 的混合 Album。混合模式有自己的 `[telegram.mixed]` 目标、断点和标题文件，旧配置缺少这些段时会自动使用公共目标和默认设置。根目录直接放置的媒体会归入一个以根目录名称命名的兼容组。
