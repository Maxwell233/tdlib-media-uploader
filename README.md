...
- 混合上传以目录下的一级子文件夹为组，递归收集其中的图片和视频；文件名排序会把数字当作自然数并按数值从大到小排列（`x.410` 在 `x.41` 前），也可改为按修改时间从旧到新。每组可设置 1–10 个媒体，超过数量会拆成多个 Album，组标题默认使用文件夹名。
...
| 扫描排序 | 修改时间或文件名（自然数字降序）二选一 | 修改时间或文件名/路径（自然数字降序）二选一 | 文件名（自然数字降序）或修改时间二选一 |
...
如果源文件位于 SMB/NAS，可在“设置与诊断”中开启“本地暂存”，或在配置中设置 `[staging] enabled = true`。程序会在发送前把已验证的文件复制到本地暂存目录，再交给 TDLib 读取；断点、标题和文件名仍以原始路径为准。暂存目录可以从缓存列表单独清理。
