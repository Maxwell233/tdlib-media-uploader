...
| 视频固定分组 | 可按扫描顺序固定分组，数量可设为 1–10 | — |
| 扫描排序 | 修改时间或文件名二选一 | 修改时间或完整路径二选一 |
| 组标题 | 可显示/隐藏 Album 或月份标题 | 编号标题可开关 |
| 文件名 | 可显示/隐藏；序号可单独开关 | 可开启，默认关闭 |
| 标题编辑 | 基础标题及追加文字 | 编号后追加文字 |
| 其他选项 | 日期读取、日期策略、媒体创建日期、视频封面 | 图片超限处理 |
