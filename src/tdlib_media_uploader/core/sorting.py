...
def natural_sort(values, *, key: Callable[[_T], object] | None = None) -> list[_T]:
    ...
    class NaturalSortKey:
        __slots__ = ("parts", "length", "index", "original")

        def __init__(self, value, index, original):
            self.parts = tuple(
                (t, v, len(r) if t == 1 else 0, r)
                for t, v, r in _natural_parts(value)
            )
        ...
        def __lt__(self, other):
            for (at, av, ar_len, ar), (bt, bv, br_len, br) in zip(
                self.parts, other.parts
            ):
                if at != bt:
                    return at < bt
                if av == bv:
                    if at == 1 and ar_len != br_len:
                        return ar_len < br_len
                    if ar != br:
                        return ar < br
                    continue
                return av < bv
    ...
    return [
        item.original for item in sorted(
            NaturalSortKey(key_func(value), index, value)
