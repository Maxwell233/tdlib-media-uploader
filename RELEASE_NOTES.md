...
本正式版合并 `1.9.3-beta1` 至 `1.9.3-beta5` 的全部功能、稳定性修复和跨平台构建成果，并完成正式发布前最终审计。

#### 最终发布前审计修复

- **备用扫描器与上传预览兼容性修复**：修复 legacy 扫描路径中的 Album key 参数错误、缺少扫描快照和旧格式分组数据不兼容，避免扫描完成后 GUI 预览出现崩溃或 `KeyError`。
- **Album identity 与配置统一**：备用扫描器现在使用真实的图片/视频/混合分组大小、source root、Album 编号和 caption key，与实际上传规划器保持一致；历史标题可以正确复用。
- **大小限制与快照一致性**：预览和 fallback 扫描使用运行时加载器中的媒体大小限制与图片压缩设置，并优先使用扫描时快照，避免文件在扫描后变化导致显示错误。
- **设置持久化修复**：修复上传验证开关、FFmpeg 压缩超时写入错误配置键的问题，同时保留旧配置键迁移读取。
- **凭据与状态安全**：空白/示例 API Hash 会在设置保存、配置加载和上传启动前被拒绝；配置错误状态支持热重载；历史记录改为原子保存，降低中断造成 JSON 损坏的风险。
- **最终回归验证**：新增正式版审计回归测试，完整离线测试集达到 323 项并全部通过。
