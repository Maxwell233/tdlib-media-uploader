...
- **上传参数快捷入口与分段导航**：在视频、图片、混合上传工作台提供“编辑上传参数”快捷按钮，一键直达设置中心对应分卷与高级配置面板，并受配置锁安全保护。
- **标题编辑体验优化**：优化预览工具栏，恢复规范的“编辑标题”按钮（带 SVG 图标与工具提示），同时支持双击与右键上下文菜单。
- **任务中心暗色体验**：修复深色模式下“当前 Album 包含的文件”列表（QListWidget）隔行背景颜色错位与文字对比度。
