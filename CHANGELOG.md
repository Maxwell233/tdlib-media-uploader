...
- 可选启用本地暂存，把 SMB/NAS 源文件在发送前复制到本地缓存；断点和标题仍使用原始路径，旧配置与旧断点格式继续兼容。
...
- 新增 `.upload_inflight` 发送日志；Album 在断点持久化前保持 `CONFIRMED` 记录，超时、断线或取消进入 `UNKNOWN`，避免下次静默重复上传。
...
- 新增 `[telegram.mixed]`、`[mixed]` 配置，以及独立的 `.mixed_state` 断点和 `.mixed_album_captions.json` 标题存储；缺少新配置时自动继承旧版公共目标和默认值。
...
- 旧版 `config.toml`、视频/图片断点、Caption 文件和上传行为保持兼容。
