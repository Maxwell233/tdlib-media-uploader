...
- 保留 1.8.5 的 Windows x64 / macOS arm64 支持和联合发布流程。
- 视频日期按 EXIF / QuickTime → 媒体创建日期 → 文件修改日期选择；ExifTool 缺失时使用现有 FFmpeg 读取容器及视频流创建日期，不使用文件系统创建时间。
- 支持 UTC、带时区和小数秒的日期，忽略无效及未设置的 QuickTime 日期；严格模式允许媒体创建日期，全部内部日期缺失才停止。
- 媒体日期按文件属性缓存；日期读取有超时，预览和上传使用同一逻辑。
- 更新预览日期来源提示、配置说明及日期优先级回归测试。
