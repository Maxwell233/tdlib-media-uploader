...
- 安全停止在当前 Album 确认、断点保存和 TDLib 关闭后生效；强制停止保留已提交但未确认的 Album，避免自动重复发送。
- 增加可配置的上传无进度 watchdog 和详细 stall 诊断；停滞 Album 进入 UNKNOWN。
- 增加 Album 边界上的 TDLib 会话轮换，并确保旧 receiver 完全退出后再创建新会话。

### 扫描与界面

- 修复上传线程完成信号早于 QThread 退出造成的扫描/配置竞态。
