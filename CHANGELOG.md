...
- **主题热切换广播机制**：支持主题切换信号向所有顶层窗口及活跃组件双向穿透广播（`refresh_theme()`），实现实时流畅重绘。
- **智能动态版本标牌**：侧边栏顶部支持智能解析语义化版本通道（`Beta` / `RC` / `Alpha` / `Dev` / `Stable`），展示更专业精致。
...
- **全量回归保障**：新增 `tests/test_beta4_gui_redesign.py` 自动化测试集，总测试用例扩充至 286 项并通过 100% 验证。
