...
- 收窄媒体扫描入口：视频仅扫描 `.mp4`（不拦截 MP4 容器中的 H.265/HEVC），图片仅扫描 `.jpg`、`.jpeg`、`.png`；旧配置中的可能被 Telegram 按文件发送的容器和图片格式会被加载时过滤。
- 新增视频缩略图截图时间配置：视频和混合上传均支持 0.01 秒精度，并可在设置中分别调整；修改截图时间后，已有封面缓存需要通过“仅清理视频封面”手动清理。
